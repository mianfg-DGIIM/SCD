...
void funcion_camarero() {
    int valor, id_filosofo, num_sentados = 0, etiq_aceptable, hay_mensaje = 0;
    MPI_Status estado;
    ...
    while (true) {
        ...
        int prox_proceso = -1;
        // prioridad para procesar peticiones más altas
        for ( int i = num_fc - 2; i >= 0 && prox_proceso == -1; i -= 2 ) {
            MPI_Iprobe( i, etiq_aceptable, MPI_COMM_WORLD, &hay_mensaje, &estado );
        ...
        if ( prox_proceso != -1 ) {
            // pedir una petición
            MPI_Recv(&valor, 1, MPI_INT, prox_proceso, MPI_ANY_TAG, MPI_COMM_WORLD, &estado);
            id_filosofo = estado.MPI_SOURCE;
