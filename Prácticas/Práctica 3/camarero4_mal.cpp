...
void funcion_filosofos(int id) {
    ...
    while (true) {
        ...
        // EXAMEN: enviar vector de entre 1 y 5 valores enteros
        // escoger tamaño de vector entre 1 y 5
        // iniciar vector con valores aleatorios entre 1 y 5
        int tamanio = aleatorio<1, 5>();
        cout << "Filósofo " << id << " solicita sentarse en mesa, envía tamaño " << tamanio << endl;
...
void funcion_camarero() {
    int valor, id_filosofo, num_sentados = 0, etiq_aceptable, propina = 0, no_ciclos_necesarios = 1;
    MPI_Status estado;
    ...
    while (true) {
        ...
        // pedir una petición
        MPI_Recv(&valor, 5, MPI_INT, MPI_ANY_SOURCE, etiq_aceptable, MPI_COMM_WORLD, &estado);
        id_filosofo = estado.MPI_SOURCE;

        // procesar mensaje
        switch (estado.MPI_TAG) {
            case TAG_SENTARSE:
                // recibir petición y sumar nº elementos del vector
                // WIP
                int tamanio_recibido;
                MPI_Get_count(&estado, MPI_INT, &tamanio_recibido);
                cout << "--[S]-- Filósofo " << id_filosofo << " se sienta en la mesa, hemos recibido tamaño " << tamanio_recibido << ", tenemos acumulada propina = " << propina << endl;
                num_sentados++;
