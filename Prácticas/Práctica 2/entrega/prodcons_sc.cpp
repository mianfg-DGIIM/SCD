 ...
 * Multihebra, monitor semántica SC, versiones FIFO y LIFO
...
/**
 * @brief Clase para monitor búfer, versión FIFO, semántica SC y múltiples
 * productores y consumidores
 */
class ProdConsSC_FIFO {
private:
    ...
    // colas de condición
    condition_variable ocupadas,    // cola donde espera consumidor
...
ProdConsSC_FIFO::ProdConsSC_FIFO() {
    primera_libre = 0;
    primera_ocupada = 0;
    num_celdas_ocupadas = 0;
}

int ProdConsSC_FIFO::leer() {
    // ganar exclusión mutua del monitor con una guarda
...
void ProdConsSC_FIFO::escribir(int valor) {
 ...
 * @param monitor: monitor productor consumidor semántica SC, versión FIFO
 * @param num_hebra: número de hebra
 */
void funcion_hebra_productora_FIFO(ProdConsSC_FIFO* monitor, int num_hebra) {
 ...
 * @param num_hebra: número de hebra
 */
void funcion_hebra_consumidora_FIFO(ProdConsSC_FIFO* monitor, int num_hebra) {
...
/**
 * @brief Clase para monitor búfer, versión LIFO, semántica SC y múltiples
 * productores y consumidores
 */
class ProdConsSC_LIFO {
private:
    static const int num_celdas_total = 10; // número de entradas del búfer
    int buffer[num_celdas_total],   // búfer
        primera_libre;              // índice de próxima inserción
...
ProdConsSC_LIFO::ProdConsSC_LIFO() {
    primera_libre = 0;
}

int ProdConsSC_LIFO::leer() {
    // ganar exclusión mutua del monitor con una guarda
    unique_lock<mutex> guarda(cerrojo_monitor);
...
void ProdConsSC_LIFO::escribir(int valor) {
    // ganar la exclusión mutua del monitor con una guarda
    unique_lock<mutex> guarda(cerrojo_monitor);
 ...
 * @param num_hebra: número de hebra
 */
void funcion_hebra_productora_LIFO(ProdConsSC_LIFO* monitor, int num_hebra) {
    for ( unsigned i = num_hebra; i < num_items; i += num_productores ) {
        int valor = producir_dato(num_hebra);
        monitor->escribir(valor);
    }
 ...
 * @param num_hebra: número de hebra
 */
void funcion_hebra_consumidora_LIFO(ProdConsSC_LIFO* monitor, int num_hebra) {
    for ( unsigned i = num_hebra; i < num_items; i += num_consumidores ) {
        int valor = monitor->leer();
        consumir_dato(valor, num_hebra);
    }
